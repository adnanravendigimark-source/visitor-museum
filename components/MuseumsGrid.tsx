...
function MuseumCard({ m }: { m: Museum }) {
  ...
  return (
    <article className="group flex flex-col rounded-2xl border border-gray-100 bg-white shadow-[0_4px_20px_rgba(0,0,0,0.04)] hover:shadow-[0_12px_30px_rgba(0,0,0,0.09)] transition-all duration-300 hover:-translate-y-1 overflow-hidden">
      {/* Card Image with Location Badge */}
      <Link href={`/${m.slug}`} className="relative aspect-[16/10] w-full overflow-hidden bg-gray-100 block">
        <SafeImage
        ...
        <div className="absolute top-3.5 left-3.5 flex items-center gap-1.5 rounded-full bg-white/90 backdrop-blur-md px-3 py-1 shadow-sm border border-white/60">
          <svg className="w-3 h-3 text-[#2D903A]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path
              strokeLinecap="round"
          ...
          <span className="text-[11px] font-semibold text-[#1F2429]">{locationLabel}</span>
        </div>
      </Link>
      ...
      <div className="flex flex-1 flex-col p-5 sm:p-6 justify-between">
        <div>
          ...
          <p className="mt-2 text-xs sm:text-[13px] text-[#54595F] leading-relaxed line-clamp-2">
            {m.cardTagline || `${m.name} in ${m.city}, ${m.country} is one of the most visited cultural landmarks.`}
          </p>
        </div>
...
export default function MuseumsGrid({
  ...
  const hasActiveFilter = !!(searchQuery || locationFilter);
  // Zero results with a filter applied and other museums actually existing
  // -> "not found, but here's everything we do have" instead of a dead
  // end. Zero results with NO filter applied (or nothing else in the
  // catalog to fall back to) is a different, much rarer case — a brand new
  // install with no museums yet — which keeps the plain empty state below.
  const showNotFoundFallback = filtered.length === 0 && hasActiveFilter && initialMuseums.length > 0;
    ...
    <section id="museums" className="py-16 sm:py-20 bg-white">
      <div className="mx-auto max-w-[1200px] px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-10 sm:mb-12 flex flex-wrap items-end justify-between gap-4">
            <div>
              <div className="flex flex-col items-start">
                <span className="text-xs font-bold tracking-[0.2em] uppercase text-[#184E3A]">{eyebrow}</span>
        ...
        {filtered.length > 0 ? (
          ...
          <div>
            {/* "Not found" panel — names exactly what didn't match, rather
                than a generic empty state, then hands the visitor the full
                catalog right below instead of a dead end. */}
            <div className="mb-10 flex flex-col items-center gap-3 rounded-2xl border border-dashed border-gray-200 bg-[#FAFAFA] p-10 text-center">
              <span className="flex h-11 w-11 items-center justify-center rounded-full bg-white text-[#184E3A] shadow-sm">
                <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
                  <circle cx="10.5" cy="10.5" r="6.5" />
                  <path d="M19.5 19.5 15.2 15.2" strokeLinecap="round" />
                </svg>
              </span>
              <p className="text-sm text-[#1F2429]">
                {searchQuery ? (
                  <>
                    No museum found for &ldquo;<span className="font-bold">{searchQuery}</span>&rdquo;
                    {locationFilter && <> in {locationFilter}</>}.
                  </>
                ) : (
                  <>No museums found in {locationFilter}.</>
                )}
              </p>
              <p className="text-xs text-gray-500">Here&apos;s everything we cover instead:</p>
              <button
                type="button"
                onClick={clearAll}
                className="mt-1 rounded-full border border-[#184E3A]/25 px-4 py-2 text-xs font-bold text-[#184E3A] transition hover:bg-[#184E3A] hover:text-white"
              >
                Clear search
              </button>
            </div>
          ...
          <div className="flex flex-col items-center gap-4 rounded-2xl border border-dashed border-gray-200 p-14 text-center">
            ...
            <p className="text-sm text-gray-500">No museums to show yet.</p>
          </div>
        )}
      </div>
    </section>
