...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  ...
  // The old slug's page, the homepage grid, and the full /museums listing
  // all need to reflect the change immediately — belt-and-suspenders on top
  // of force-dynamic + no-store, same reasoning as every other
  // content-saving route. A city rename also changes which Other
  // Attractions this museum shows, but that section reads live on every
  // request (no cache to bust) — see lib/otherAttractions.ts.
