...
import { getAllOtherAttractions } from "@/lib/otherAttractions";

export const dynamic = "force-dynamic";

export default async function AdminAllAttractionsPage() {
  const [museums, allAttractions] = await Promise.all([getMuseums(), getAllOtherAttractions()]);

  const countByMuseum = new Map<string, number>();
  for (const a of allAttractions) {
    countByMuseum.set(a.museumId, (countByMuseum.get(a.museumId) || 0) + 1);
  }
  ...
  return (
    <div>
      ...
      <h1 className="mt-3 font-display text-2xl font-bold text-stone-900">Other Attractions</h1>
      <p className="mt-1 text-sm text-stone-600">
        Pick a museum to see and edit the "Other Attractions" cards shown on its own page — name, description,
        price, image, and booking link, added and managed per museum, same as its Tours &amp; Tickets.
      </p>
